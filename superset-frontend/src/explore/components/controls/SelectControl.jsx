...
export default class SelectControl extends React.PureComponent {
  ...
  getOptions(props) {
    ...
    if (props.allowAll === true && props.multi === true && (props.options?.length || 0) > 0) {
  ...
  component() {
    ...
    const placeholder =
      this.props.placeholder || t('%s option(s)', this.state.options.filter(option => !option.meta).length);
    const selectProps = {
    ...
    if (this.props.freeForm) {
      ...
      selectProps.selectComponent = StyledSelect;
    }
    return (
        ...
        <OverlayTrigger
          ...
          overlay={(
            <Tooltip id={'error-tooltip'}>
              {this.props.disabledReasons?.map(reason => (<div>{reason}</div>))}
            </Tooltip>
          )}
        >
          <div>
