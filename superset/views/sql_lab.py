...
class SavedQueryFilter(BaseFilter):  # pylint: disable=too-few-public-methods
    def apply(self, query: BaseQuery, value: Callable) -> BaseQuery:
        """
        Filter saved queries to only those owned by current user. If
        can_access_all_queries permission is set a user can list all saved queries

        :returns: query
        """

        if not security_manager.can_access_all_queries():
            query = query.filter(SavedQuery.user_id == g.user.get_user_id())
        return query
...
class SavedQueryViewApi(SavedQueryView):  # pylint: disable=too-many-ancestors
    ...
    add_columns = ["label", "db_id", "schema", "description", "sql", "extra_json", "table_id"]
    edit_columns = add_columns
...
class TabStateView(BaseSupersetView):
    ...
    def post(self):  # pylint: disable=no-self-use
        ...
        tab_state = TabState(
            ...
            label=query_editor.get("title", "Untitled"),
            active=True,
