...
from superset.utils.core import QueryStatus, TimeRangeEndpoint, get_example_database
from superset.utils.fulfillment import get_token_auth, get_api_key
...
def get_form_data(slice_id=None, use_slice_data=False):
    ...
    if url_id:
        saved_url = db.session.query(models.Url).filter_by(urlsafe=url_id).first()
        if saved_url:
...
def check_datasource_has_non_empty_data(datasource_type, datasource_id, form_data) -> bool:
    viz_type = form_data.get("viz_type")
    if datasource_type == 'table' and (viz_type == 'regression_prediction' or viz_type == 'classification_prediction'):
        predicted_column = form_data.get("prediction")
        table = db.session.query(SqlaTable).filter_by(id=datasource_id).one_or_none()
        if table:
            from_table = f'"{table.schema}"."{table.table_name}" ' if table.schema else f'"{table.table_name}"'
            query = "SELECT " + f'"{predicted_column}"' + " AS predicted FROM " +  from_table + \
                    f'WHERE "{predicted_column}" IS NULL LIMIT 1 '
            df = table.database.get_df(query, table.schema)
            return df.empty


def get_savedquery_by_table_id(datasource_id: int, session: Session) -> "SavedQuery":
    return (
        session.query(SavedQuery).filter_by(table_id=datasource_id).one_or_none()
    )
