...
def validate_json_metadata(value):
    ...
    try:
        ...
        raise ValidationError("JSON not valid")
    # marshmallow 3.x change https://marshmallow.readthedocs.io/en/stable/upgrading.html#schemas-are-always-strict
...
class BaseDashboardSchema(Schema):
    # pre_load change in 3.x https://marshmallow.readthedocs.io/en/stable/upgrading.html#decorated-methods-and-handle-error-receive-many-and-partial
    @pre_load
    def pre_load(self, data, **kwargs):  # pylint: disable=no-self-use
        if data.get("slug"):
