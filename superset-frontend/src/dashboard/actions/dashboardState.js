...
export function togglePublic(isPublic) {
  return { type: TOGGLE_PUBLIC, isPublic };
}

export function savePublic(id, isPublic) {
  return function savePublicThunk(dispatch) {
    return SupersetClient.put({
      endpoint: `/api/v1/dashboard/${id}`,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
      ...
      .then(() => {
        const nowPublic = isPublic ? 'public' : 'unpublic';
        dispatch(addSuccessToast(t(`This dashboard is now ${nowPublic}`)));
        dispatch(togglePublic(isPublic));
      })
      .catch(() => {
        dispatch(
          addDangerToast(
  ...
  return (dispatch, getState) => {
    if (!interval) {
      chartList.forEach(chartKey =>
        dispatch(refreshChart(chartKey, force, isPublic, dashboardId)),
      );
      return;
    }
