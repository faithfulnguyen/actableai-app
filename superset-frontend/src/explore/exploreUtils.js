...
import { gtagEvent } from '../utils/googleAnalyticsTracking';
...
export function getURIDirectory(
  ...
  if (
    ['json', 'csv', 'query', 'results', 'samples', 'table'].indexOf(
      endpointType,
    ) >= 0
  ) {
    if (isPublic) {
      directory = 'superset/explore_json_public/' + dashboardId + '/';
    } else {
...
export function getExploreUrlAndPayload({
  ...
  const directory = getURIDirectory(
    formData,
    endpointType,
    isPublic,
    dashboardId,
...
export function exportChart(formData, endpointType, target) {
  if (['csv', 'table'].indexOf(endpointType) >= 0) {
    gtagEvent('analytics', 'User-download-' + endpointType, window.location.href);
  }
  formData.taskId =
    formData.taskId === undefined || formData.taskId === null
      ? localStorage.getItem('tempTaskId')
      : formData.taskId;
