...
import { addDangerToast, addWarningToast } from '../messageToasts/actions';
...
export const CHART_UPDATE_DATA = 'CHART_UPDATE_DATA';
export function chartUpdateData(data, key) {
  return { type: CHART_UPDATE_DATA, data, key };
}

export const CHART_UPDATE_PENDING = 'CHART_UPDATE_PENDING';
export function chartUpdatePending(queryResponse, key) {
  return { type: CHART_UPDATE_PENDING, queryResponse, key };
}
...
export function setPollingStatus(key, status) {
  return { type: SET_POLLING_STATUS, key, status };
}

export const SET_TASK_ID = 'SET_TASK_ID';
export function setTaskId(key, value) {
  return { type: SET_TASK_ID, key, value };
}

export const CHART_POLLING_RENDERED = 'CHART_POLLING_RENDERED';
export function setChartRendered(key, status) {
  return { type: CHART_POLLING_RENDERED, key, status };
}
    ...
    const queryPromise = clientMethod(querySettings)
      .then(({ json }) => {
        ...
        if (json.warning_message) {
          dispatch(addWarningToast(t(json.warning_message)));
        }
...
export function getSavedChart(
   ...
   *
   */
  return exploreJSON(formData, force, timeout, key, 'GET', false, dashboardId);
}
...
export function refreshChart(chartKey, force, isPublic, dashboardId) {
...
export function updateTaskid(chartId, taskId) {
  axios.put(`/api/chart/${chartId}`, {
    taskId,
  });
}
