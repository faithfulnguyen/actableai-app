...
from superset import ConnectorRegistry, db, is_feature_enabled, security_manager, cache
...
class Slice(
    ...
    # will render
        ...
        name = ""
        if viz_type in object_name.keys():
            name = object_name[viz_type]
        return Markup(name)
    ...
    @property
    def saved_query_id(self) -> int:
        saved_query = db.session.query(SavedQuery).filter_by(table_id=self.datasource_id).one_or_none()
        if saved_query:
    ...
    @property
    def can_edit(self):
        return security_manager.check_access_permission("can_edit", self.id, self.created_by_fk, 'SliceModelView', Slice)

    @property
    def can_delete(self):
        return security_manager.check_access_permission("can_delete", self.id, self.created_by_fk, 'SliceModelView', Slice)
    ...
    def created_by_url(self) -> str:
        if not security_manager.check_role_user('admin') and self.is_example:
            return ""
        
        return f"/superset/profile/{self.created_by.username}" if self.created_by else ""
