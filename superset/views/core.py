...
from superset.connectors.sqla.models import AnnotationDatasource, TableColumn
...
from superset.views.utils import get_dashboard_extra_filters, check_datasource_has_non_empty_data, check_resource_permissions, check_datasource_perms
...
class R(BaseSupersetView):
    ...
    def index(self, url_id):
        try:
            url = db.session.query(models.Url).filter_by(urlsafe=url_id).first()
            if url and url.url:
                ...
                if url.url.startswith(explore_url):
                    ...
                    m = re.match(r".+/dashboard/([0-9a-zA-Z-]+)", url.url)
                    dash_id = m.group(1)
                    if dash_id.isnumeric():
                        dash = db.session.query(Dashboard).filter_by(id=dash_id).first()
                    elif is_valid_uuid(dash_id):
                        dash = db.session.query(Dashboard).filter_by(uuid=dash_id).first()
                    else:
                        dash = db.session.query(Dashboard).filter_by(slug=dash_id).first()
                    if dash:
                        if g.user.is_anonymous:
                            dashboard_url = "/dashboard/public/" + f"{dash.uuid}"
                        else:
                            dashboard_url = "/superset/dashboard/" + f"{dash.uuid}"
    ...
    def shortner(self):
        ...
        m = re.match(r".+/dashboard/([0-9a-zA-Z-]+)", url)
        if m is not None:
            dash_id = m.group(1)
            if dash_id.isnumeric():
                dash = db.session.query(Dashboard).filter_by(id=dash_id).first()
                ...
                dash = db.session.query(Dashboard).filter_by(slug=dash_id).first()
            if dash and dash.is_public:
                obj = models.Url(url=url, urlsafe=secrets.token_urlsafe(64))
            else:
                obj = models.Url(url=url, urlsafe=secrets.token_urlsafe(16))
        else:
            obj = models.Url(url=url, urlsafe=secrets.token_urlsafe(16))
        db.session.add(obj)
        db.session.commit()
        short_link = obj.urlsafe
        return Response(
            "{request.headers[origin]}/r/{short_link}".format(
                scheme=request.scheme, request=request, short_link=short_link
            ),
            mimetype="text/plain",
        )
...
class Superset(BaseSupersetView):
    ...
    def generate_json(
            self, viz_obj, csv=False, query=False, results=False, samples=False, table=False, has_non_empty_data=False
    ):
        try:
            if csv:
                get_excel_output = getattr(viz_obj, "get_excel_output", None)
                if callable(get_excel_output):
                    return send_file(viz_obj.get_excel(), attachment_filename='output.xlsx', as_attachment=True)
            ...
            if table and "save_table_name" in viz_obj.form_data and "database_name" in viz_obj.form_data and "schema_name" in viz_obj.form_data:
                tbl_name = viz_obj.form_data["save_table_name"]
                db_name = viz_obj.form_data["database_name"]
                schema_name = viz_obj.form_data["schema_name"]
                database = utils.get_db_by_name(db_name)
                if "taskId" in viz_obj.form_data and viz_obj.form_data["taskId"] is not None:
                    data_string = viz_obj.get_csv_custom()
                else:
                    data_string = viz_obj.get_csv()
                data = io.StringIO(data_string)
                df = pd.read_csv(data, sep=",")
                viz_obj.create_table_from_df(tbl_name, database, schema_name, df)
                ...
                if not security_manager.can_access_database(database):
                    raise Exception('User can not access database')
                tbl.database = database
                schemas = security_manager.get_access_schemas(database.id)
                if schema_name not in schemas:
                    raise  Exception('User can not access schema')
                tbl.schema = schema_name
                db.session.merge(tbl)
                db.session.commit()
                tbl.fetch_metadata()
                return redirect(f'/superset/explore/table/{tbl.id}')

            payload = viz_obj.get_payload()
            if has_non_empty_data:
                payload["warning_message"] = "As predicted column doesn't have empty cells, no predicted results will be " \
                                            "returned and only performance on validation data is shown."
    ...
    def explore_json(self, datasource_type=None, datasource_id=None):
        ...
        table = request.args.get("table") == "true"
        form_data = get_form_data()[0]
        ...
        if is_enabled_billing() and not allow_to_run_analytics(form_data["viz_type"], datasource_id, datasource_type):
            return json_error_response("You have not paid or the available time has expired", status=403)
        ...
        has_non_empty_data = check_datasource_has_non_empty_data(datasource_type, datasource_id, form_data)

        return self.generate_json(
            viz_obj, csv=csv, query=query, results=results, samples=samples, table=table, has_non_empty_data=has_non_empty_data
        )

    @expose("/explore_json_public/<dashboard_id>/", methods=EXPLORE_JSON_METHODS)
    def explore_json_public(self, dashboard_id):
        datasource_id = None
        ...
        if not dash:
            return json.dumps({"error": "invalid dashboard"}), 400
        if not dash.is_public:
            return json.dumps({"error": "dashboard is not publish"}), 500
        ...
        has_slice_in_dashboard = qry.filter(Dashboard.slices.any(id=slice_id)).one_or_none()

        if has_slice_in_dashboard is None:
            return json.dumps({"error": "this chart is not belong current dashboard"}), 500
    ...
    def get_info(self, datasource_type=None, datasource_id=None):
        ...
        columnsData = []
        columns = datasource.data['columns']
        for column in columns:
            if column['type'] is not None:
                if column['type'].find('VARCHAR') != -1 or column['type'].find('TEXT') != -1:
                    data = datasource.values_for_column(
                        column['column_name'])
                    for value in data:
                        columnsData.append(column['column_name'] + ":" + value.replace(' ', '_'))
                elif column['type'].find('INT') != -1 or column['type'].find('FLOAT') != -1 or column['type'].find(
                        'PRECISION'):
                    columnsData.append(column['column_name'])

        return self.json_response(columnsData)
    ...
    def explore(self, datasource_type=None, datasource_id=None):
        ...
        if datasource_type == 'chart':
            error_redirect = "/chart/list/"
        elif datasource_type == 'table':
            error_redirect = "/tablemodelview/list/"
        else:
            error_redirect = '/'
        ...
        if action == "overwrite" and not slice_overwrite_perm and "admin" not in user_roles:
        ...
        columns = datasource.data['columns']
        for column in columns:
            data = datasource.values_for_column(column['column_name'])
            inValue = []
            for value in data:
        ...
        if data["sql"] is not None:
            saved_query = get_savedquery_by_table_id(data["id"], db.session)
            if saved_query:
        ...
        dbs_arr = []
        if not config["ALLOW_GUEST_VIEW_PUBLIC_DATA"] or not g.user.is_anonymous:        
        ...
        bootstrap_data = {
            ...
            "can_overwrite": "admin" in user_roles or slice_overwrite_perm,
            "datasource": data,
            "form_data": form_data,
            "datasource_id": datasource_id,
            "datasource_type": datasource_type,
            ...
            "common": common_bootstrap_payload(True) if g.user.is_anonymous else common_bootstrap_payload(),
            "env": env_config,
            "dbs": dbs_arr,
            "has_admin_role": "admin" in user_roles,
            'user_email': "guest" if g.user.is_anonymous else g.user.email
        }
        table_name = (
    ...
    def save_or_overwrite_slice(
        ...
        response = {
            "can_add": slice_add_perm,
            "can_download": slice_download_perm,
            "can_overwrite": "admin" in user_roles or is_owner(slc, g.user),
    ...
    def schemas(self, db_id, force_refresh="false"):
        schemas = security_manager.get_access_schemas(db_id, force_refresh)
    ...
    @api
    @handle_api_exception
    @expose("/search_user", methods=["GET"])
    def query_search_user(self):
        ...
        if email:
            ...
            search_user = db.session.query(User).filter_by(email=email).one_or_none()
            if search_user:
                return json_success(json.dumps({'id': search_user.id, 'email': search_user.email}, default=utils.json_int_dttm_ser))
    ...
    if config['DASHBOARD_FEATURE'] == 'true':
        @api
        @has_access_api
    ...
    @expose("/sqllab_viz/edit/<datasource_id>", methods=["PUT"])
    @event_logger.log_this
    def sqllab_viz_update(self, datasource_id):
        SqlaTable = ConnectorRegistry.sources["table"]
        data = json.loads(request.form.get("data"))
        table_name = data.get("datasourceName")
        database_id = data.get("dbId")
        ...
        if table:
            db.session.query(TableColumn) \
                .filter(TableColumn.table == table) \
            ...
            for config in data.get("columns"):
                column_name = config.get("name")
                SqlaTable = ConnectorRegistry.sources["table"]
                TableColumns = SqlaTable.column_class
                col = TableColumns(
                    column_name=column_name,
                    filterable=True,
                    groupby=True,
            ...
            return json_error_response('table not found', 500)
    ...
    def sql_json_exec(
        ...
        accessible_schema = security_manager.get_access_schemas(database_id)
        if schema is None:
            return json_error_response(f"No schema selected")
        elif schema not in accessible_schema:
            return json_error_response(f"Schema {schema} does not exist or permission denied")
        
        parsed_query = ParsedQuery(sql)
        for table in parsed_query.tables:
            if "." in table and len(table.split(".")) == 2:
                schema_from_editor, _ = table.split(".")
                if schema_from_editor not in accessible_schema:
                    return json_error_response(f"Schema {schema_from_editor} does not exist or permission denied")
    ...
    def handle_exception(e):
        return (
            render_template("superset/404.html", error_msg=get_error_msg()),
            404,
        )
    ...
    def handle_exception(e):
        logger.exception(e)
        return (
            render_template("superset/500.html", error_msg=get_error_msg()),
            500,
        )
    ...
    def handle_exception(e):
        ...
        logger.exception('Error view %s' % dest)
        logger.exception(e)
    ...
    def welcome(self):
        ...
        payload = {
            ...
            "env": {"DASHBOARD_FEATURE": app.config.get('DASHBOARD_FEATURE'),
                    },
        }
    ...
    def profile(self, username):
        ...
        if not config['ALLOW_VIEW_ANOTHER_PROFILE']:
    ...
    def schemas_access_for_csv_upload(self):
        ...
        try:
            ...
            allowed_schemas = security_manager.get_schemas_for_csv_upload(db_id)
            return self.json_response(allowed_schemas)
        except Exception as e:
            logger.exception(e)
            return json_error_response(
