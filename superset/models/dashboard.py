...
class Dashboard(  # pylint: disable=too-many-instance-attributes
    ...
    uuid = Column(UUID(as_uuid=True), default=uuid.uuid4, unique=True, nullable=True)
        ...
        if self.json_metadata:
            ...
            if default_filters and default_filters != "{}":
                try:
                    ...
                    pass
        return f"/superset/dashboard/{self.uuid}/"
    ...
    @property
    def can_edit(self):
        return security_manager.check_access_permission("can_edit", self.id, self.created_by_fk, 'DashboardModelView', Dashboard)

    @property
    def can_delete(self):
        return security_manager.check_access_permission("can_delete", self.id, self.created_by_fk, 'DashboardModelView', Dashboard)
