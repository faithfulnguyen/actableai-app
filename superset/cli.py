...
from superset.utils import core as utils, nifi
...
def set_is_example_datasets():
    from superset.connectors.sqla.models import SqlaTable

    datasets = db.session.query(SqlaTable).all()
    for dataset in datasets:
        if security_manager.datasource_can_access_by_beta(dataset):
...
def load_examples_run_v2(load_test_data, only_metadata=False, force=False):
    if only_metadata:
        print("Loading examples metadata")
    else:
        examples_db = utils.get_example_database()
    ...
    print("Creating slice [World health population by year]")
    examples.load_world_health_population_by_year(only_metadata, force)

    print("Creating slice [Time series of retail sales]")
    examples.load_retail_sales_data(only_metadata, force)

    print("Creating slice [Individual - Correlation]")
    examples.load_invidiual_data(only_metadata, force)

    print("Creating slice [Predict Rental Price]")
    examples.load_apartment_data(only_metadata, force)
...
def load_examples_v2(load_test_data, only_metadata=False, force=False):
    """Loads a set of Slices and Dashboards and a supporting dataset """
    load_examples_run_v2(load_test_data, only_metadata, force)


@with_appcontext
@superset.command()
def run_set_is_example_datasets():
    set_is_example_datasets()
    
    
@with_appcontext
@superset.command()
def init_billing():
    """Init data of billing"""
    from superset.billing.utils import is_enabled_billing, is_enabled_trial_billing

    if is_enabled_billing() and is_enabled_trial_billing():
        print("Seeding trial for existing users")
