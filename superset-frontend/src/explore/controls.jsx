 ...
 * - label: the label as stimeseries_limit_metrichown in the control's header
...
function columnPositiveGroup(datasource, controls) {
  if (datasource && datasource.columns_positive) {
    if (controls && controls.columns_name) {
      if (controls.columns_name.value) {
        const columnsValue = controls.columns_name.value;
        return datasource.columns_positive
          .filter(c => {
            const value =
              c.indexOf(':') !== -1 ? c.substr(0, c.indexOf(':')) : c;
            return !columnsValue.includes(value);
          })
          .map(col => [col]);
      }
    }
    return datasource.columns_positive.map(col => [col]);
  }
  return [];
}

function columnGroup(datasource, controls) {
  return datasource.columns_name.filter(columnName => {
    return columnName[0] !== JSON.parse(controls.columns_values?.value || '{}').column
  }).map(col => [col]);
}

function columnValues(datasource, controls) {
  if (datasource && datasource?.columns_values) {
    return datasource?.columns_values?.map(col => [
      JSON.stringify({
        column: col.column,
        value: col.value,
      }),
      col.value ? `${col.column}:${col.value}` : col.column,
    ]);
  }
  return [];
}

function getValuesFromColumn(datasource, columnName) {
  return datasource?.group_values[columnName]?.map(col => [
    col.value,
    col.value,
  ]) || [];
}

function groupValues(datasource) {
  if (datasource && datasource.group_values) {
    return datasource.group_values;
  }
  return {};
}
...
export const controls = {
  ...
  biased_groups: {
    type: 'SelectControl',
    multi: true,
    label: t('Sensitive Groups'),
    default: [],
    description: t('Groups that are protected against biases.'),
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    valueKey: 'column_name',
    allowAll: true,
  ...
  debiased_features: {
    type: 'SelectControl',
    multi: true,
    label: t('Proxy Features'),
    default: [],
    description: t('Features that are correlated with sensitive groups and need to be de-biased.'),
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    valueKey: 'column_name',
    allowAll: true,
  ...
  extra_columns: {
    ...
    default: [],
    description: t('Columns that are displayed along the returned result but not used in the analysis'),
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    valueKey: 'column_name',
    mapStateToProps: state => ({
  ...
  treatment: {
    type: 'SelectControl',
    label: t('Treatment'),
    description: t('Variable that causes the effect of interest'),
    allowAll: true,
    multi: false,
  ...
  treatment_control: {
    type: 'SelectControl',
    multi: false,
    label: t('Treatment control'),
    default: null,
    description: t('Treatment value for control group, e.g. placebo'),
    allowAll: true,
    mapStateToProps: state => ({
      choices: getValuesFromColumn(
        state.datasource,
        state.controls?.treatment?.value,
      ),
    }),
    commaChoosesOption: false,
    freeForm: false,
  ...
  positive_outcome_value: {
    type: 'SelectControl',
    multi: false,
    label: t('Positive Outcome Value'),
    default: null,
    description: t('This value is treated as 1, other values are treated as 0'),
    allowAll: true,
    mapStateToProps: state => ({
      choices: getValuesFromColumn(
        state.datasource,
  ...
  new_treatment: {
    type: 'SelectControl',
    label: t('New treatment'),
    description: t('New treatment values for counterfactual predictions'),
    allowAll: true,
    multi: false,
    valueKey: 'column_name',
    optionRenderer: c => <ColumnOption column={c} showType />,
  ...
  effect_modifier: {
    type: 'SelectControl',
    label: t('Effect modifier'),
    description: t('If selected, causal effect will be estimated at the observed values of this variable'),
    default: null,
    multi: false,
    commaChoosesOption: false,
    freeForm: false,
  ...
  common_causes: {
    type: 'SelectControl',
    multi: true,
    label: t('Common causes'),
    default: [],
    description: t('Common causes that might affect both treatments and outcome'),
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    valueKey: 'column_name',
    allowAll: true,
  ...
  log_treatment: {
    type: 'CheckboxControl',
    label: t('Logarithmic treatment'),
    default: false,
    description: t('If checked, treatment values will be displayed in log scale'),
  },

  log_outcome: {
    type: 'CheckboxControl',
    label: t('Logarithmic outcome'),
    default: false,
    description: t('If checked, outcome values will be displayed in log scale'),
  },
  ...
  duration: {
    ...
    description: t('Duration of timeline'),
  },
  ...
  timeline: {
    ...
    validators: [v.nonEmpty],
    description: t(
      'Defines the timeline. ' + 'This defines the timeline on the chart',
    ),
  },
  ...
  adhoc_filters: {
    ...
    label: t('Filters'),
    default: null,
    description: 'If set, only filtered data are applied',
    mapStateToProps: state => ({
  ...
  // begin add controls chart prediction - 2019/11/05 kyvd
  date_time: {
    ...groupByControl,
    label: 'Date time',
    multi: false,
    ...
    mapStateToProps: (state, control) => {
      ...
      if (state.datasource) {
        newState.options = state.datasource.columns.filter(c =>
          type.includes(c.type),
        );
        if (control && control.includeTime) {
          newState.options.push(timeColumnOption);
        }
    ...
    description: t('Defines the date time.'),
  },

  min_freq: {
    label: t('Minimum Frequency'),
    description: t('Only show keywords with this minimum mentions'),
    type: 'TextControl',
    isInt: true,
    default: 5,
    renderTrigger: true,
  ...
  prediction: {
    ...
    mapStateToProps: (state, control) => {
      const newState = {};
      if (state.datasource) {
        newState.options = state.datasource.columns.filter(c => c.type);
        if (control && control.includeTime) {
          newState.options.push(timeColumnOption);
        }
      }
    ...
    description: t('Select the column to be predicted.'),
  },
  ...
  outcome: {
    ...
    description: t('Column that indicates treatment outcomes'),
  },

  prediction_day: {
    type: 'TextControl',
    default: 1,
    label: t('Number of predicting days'),
    validators: [v.integer, v.greater0],
    description: t('Number of predicting days.'),
  },
  // end add controls chart prediction - 2019/11/05 kyvd

  positive_group: {
    type: 'SelectControl',
    label: t('Positive Group'),
    description: t('Choose positive group'),
    mapStateToProps: state => ({
      choices: columnPositiveGroup(state.datasource, state.controls),
    }),
    validators: [v.nonEmpty],
    default: null,
  },
  ...
  correlation_target: {
    type: 'SelectControl',
    label: t('Correlation target'),
    default: null,
    description: t('Target column for correlation'),
    valueKey: 'column_name',
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    mapStateToProps: state => ({
  ...
  correlation_target_value: {
    type: 'SelectControl',
    label: t('Correlation target value'),
    description: t('Target column value for correlation'),
    default: null,
    multi: false,
    allowAll: true,
    commaChoosesOption: false,
    ...
    mapStateToProps: state => ({
      choices: getValuesFromColumn(
        state.datasource,
        state.controls?.correlation_target?.value,
      ),
    }),
    validators: [v.nonEmpty],
  },
  ...
  correlation_control: {
    type: 'SelectControl',
    label: t('De-correlation'),
    default: [],
    description: t('If set, data are sampled to reduce correlation between the target and selected column'),
    multi: true,
    mapStateToProps: state => ({
      choices: columnValues(state.datasource, state.controls),
    }),
  },

  dictint: {
    type: 'CheckBoxControlCustom',
    label: t('Dictint'),
    renderTrigger: false,
    description: t('Dictint'),
    default: false,
  },
  ...
  horizontal_mode: {
    type: 'CheckboxControl',
    label: t('Horizontal mode'),
    default: false,
    renderTrigger: true,
  },

  row_limit_correlation: {
    type: 'TextControl',
    label: t('Row limit'),
    default: 10000,
    validators: [v.integer, v.greater0],
    description: t('Number of limited rows'),
  },

  number_factors: {
    type: 'TextControl',
    label: t('Number of displayed factors'),
    default: 20,
    validators: [v.integer, v.greater0],
  },
  ...
  ratio: {
    ...
    dontRefreshOnChange: true,
    description: t(
      'The percentage of rows with non-empty predicted target that are used for validation of trained models (the rest are used for training), typically from 5-20.',
    ),
  },
  ...
  perplexity: {
    type: 'TextControl',
    default: 40,
    label: t('Perplexity value'),
    validators: [v.integer, v.from1to100],
    description: t('TSNE Perplexity'),
  },
  ...
  hover: {
    ...
    description: 'Values of these columns will be displayed in the tooltip',
    default: 'cluster_id',
    optionRenderer: c => <ColumnOption column={c} showType />,
    valueRenderer: c => <ColumnOption column={c} />,
    valueKey: 'column_name',
    allowAll: true,
    mapStateToProps: state => {
      let datasource = state.datasource ? state.datasource.columns : [];
      return {
        options: [{
          id: 0,
          ...
          verbose_name: null,
          description: null,
          expression: 'Display color group by number of clusters',
          filterable: true,
          groupby: true,
          is_dttm: false,
          type: null,
  ...
  number_clusters: {
    ...
    description: t('Number of segmentation groups.'),
  },

  num_trials: {
    type: 'SliderControl',
    label: t('Number of trials'),
    default: 8,
    step: 1,
    min: 1,
    max: 20,
    description: t(
      'Number of trials for hyper-parameter optimisation. Increasing the number of trials usually results in better forecast but longer training time.',
    ),
  },
  ...
  rules: {
    type: 'Rules',
    label: t('Denial constraints'),
    description: t(
      'Define a set of constraints to detect invalid values. These values will be fixed automatically.',
    ),
    mapStateToProps: (state, control) => {
      const datasource = state.datasource ? state.datasource.columns : [];
      const options = datasource.map(function(row, i) {
        return {
      ...
      return {
        options: options,
        default: [],
        choices: groupValues(state.datasource),
      };
    },
  },
  ...
  show_empty: {
    type: 'CheckboxControl',
    label: t('Impute empty or null cells'),
    renderTrigger: false,
    default: false,
  },
  ...
  polynomial_degree: {
    type: 'SliderControl',
    label: t('Polynomial degree'),
    default: 1,
    step: 1,
    min: 1,
    max: 20,
    description: t('Exponents of numeric variables and cross intersection limit of variables'),
  },

  regression_quantile_low: {
    type: 'TextControl',
    label: t('Quantile low'),
    default: 5,
    isInt: true,
    validators: [v.integer, v.from1to100],
    description: t('If set, a lower bound with the set confidence (in percentage) is returned.'),
  },
  ...
  priors: {
    ...
    mapStateToProps: (state, control) => {
      const datasource = state.datasource ? state.datasource.columns : [];
      const columns = datasource.map(function(row, i) {
        return {
          value: row.column_name,
          label: row.column_name,
          type: row.type
  ...
  quantile_mode: {
    type: 'CheckboxControl',
    label: t('Prediction interval'),
    default: false,
    renderTrigger: false,
    description: 'Also generate prediction range from Quantile low to Quantile high (e.g. 5%-95% for 90% prediction interval)',
  },

  optimization_presets: {
    type: 'CheckboxControl',
    label: t('Optimize for quality'),
    default: false,
    renderTrigger: false,
    description: 'Optimize for quality of predictions, usually taking significantly longer time',
  },

  explain_predictions: {
    type: 'CheckboxControl',
    label: t('Explain predictions'),
    default: false,
    renderTrigger: false,
    description: 'For regression, show contribution of each feature to every prediction (SHAP values). For classification, explain predictions in natural language.',
  },
  ...
  max_train_samples: {
    type: 'TextControl',
    default: 5000,
    label: t('Max train samples'),
    validators: [v.integer, v.greater0],
    description: t('Maximum number of random samples used for training. Higher values can generate better results ' +
                   'but also take significant longer time to run'),
  },

  cross_validation: {
    type: 'CheckboxControl',
    label: t('Cross validation'),
    default: false,
    renderTrigger: false,
    description: 'Splitting data sets into folds and cross-validate the trained models instead of using a held-out set',
  },

  kfolds: {
    type: 'SliderControl',
    label: t('Number of folds'),
    default: 10,
    step: 1,
    min: 2,
    max: 20,
    validators: [v.integer, v.from1to100],
    description: t('Number of cross-validation folds'),
  },

  option_regression: {
    type: 'MoreOption',
    option: ['cross_validation', 'kfolds'],
    show: false,
  },
};
