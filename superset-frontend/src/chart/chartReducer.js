...
export default function chartReducer(charts = {}, action) {
  const actionHandlers = {
    ...
    [actions.CHART_UPDATE_DATA](state = {}) {
      return {
        ...state,
        queryResponse: {
          ...state.queryResponse,
    ...
    [actions.CHART_UPDATE_PENDING](state) {
      return {
        ...state,
        chartStatus: 'pending',
      };
    ...
    [actions.CHART_RENDERING_FAILED](state) {
      return {
        ...
        chartAlert: t(
          action.error ||
            'Sorry, something went wrong. We are already notified about this, please try again later.',
        ),
      };
    },
    ...
    [actions.CHART_UPDATE_FAILED](state) {
      let error  = t('Network error.');
      if(action.queryResponse.error) {
        error = action.queryResponse.error;
      }
      else if (action.queryResponse.errors) {
        error = action.queryResponse.errors.map(item=> item.message).join("\n ");
      }
      return {
        ...state,
        chartStatus: 'failed',
    ...
    [actions.SET_POLLING_STATUS](state) {
      return {
        ...state,
        isPolling: action.status,
      };
    },
    [actions.SET_TASK_ID](state) {
      return {
        ...state,
        taskId: action.value,
      };
    },
    [actions.CHART_POLLING_RENDERED](state) {
      return {
        ...state,
        isRendered: action.status,
      };
    },
  };
