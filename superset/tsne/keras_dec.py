...
logging.info("Loaded DEC")

class ClusteringLayer(Layer):
    def __init__(self, output_dim, input_dim=None, learning_rate=None, weights=None, alpha=1.0, **kwargs):
...
class DeepEmbeddingClustering(object):
    ...
    def initialize(self, X, save_autoencoder=False, layerwise_pretrain_iters=50000, finetune_iters=100000):
        if self.pretrained_weights is None:
            ...
            iters_per_epoch = int(len(X)/self.batch_size) + 1
            print(len(X))
    ...
    def cluster(self, X, y=None,
        ...
        logger.info('Update interval %d', update_interval)

        if save_interval is None:
            # 50 epochs
            save_interval = X.shape[0]/self.batch_size*50
        logger.info('Save interval %d', save_interval)
        ...
        while train:
            ...
            if iter_max < iteration:
                logger.info('Reached maximum iteration limit. Stopping training.')
                return self.y_pred
            ...
            if iteration % update_interval == 0:
                ...
                if y is not None:
                    ...
                    logger.info('Iteration '+str(iteration)+', Accuracy '+str(np.round(acc, 5)))
                else:
            ...
            if iteration % save_interval == 0:
                z = self.DEC.get_layer(index=0).predict(X)
                break
