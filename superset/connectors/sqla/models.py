...
class SqlaTable(Model, BaseDatasource):
    ...
    def get_real_type(self):
        return "sql" if self.is_sqllab_view else "table"
    ...
    @property
    def can_edit(self):
        return security_manager.can_edit_datasource(table=self)

    @property
    def can_delete(self):
        return security_manager.can_delete_datasource(table=self)
    ...
    def created_by_name(self) -> str:
        ...
        if self.is_example:
            return "Actable AI"
        if self.is_public and (g.user.is_anonymous or g.user.id != self.created_by_fk):
            return "Anonymous"
    ...
    def created_by_url(self) -> str:
        ...
        if not security_manager.check_role_user('admin') and (
            self.is_example 
            or (self.is_public and (g.user.is_anonymous or g.user.id != self.created_by_fk))
        ):
            return ""
    ...
    @property
    def free_to_run_analytics(self) -> bool:
        return self.is_example or self.is_public
    
    def set_is_example(self, value=True):
        self.is_example = value
        security_manager.add_remove_datasource_to_beta_role(
            self, self.is_example)
        
    def set_is_public(self, value=True):
    ...
    def values_for_column(self, column_name: str, limit: int = 10000) -> List:
        ...
        sql = "{}".format(
            qry.compile(engine, compile_kwargs={"literal_binds": True}, dialect=postgresql.dialect(paramstyle="named")))
    ...
    def fetch_metadata(self, commit=True) -> None:
        ...
        db.session.query(TableColumn) \
            .filter(TableColumn.table == self) \
            .filter(TableColumn.column_name.notin_([col.name for col in table.columns])) \
    ...
    def query_datasources_by_name(
        ...
        if schema:
            if schema == 'public':
                query = query.filter(or_(cls.schema == schema, cls.schema.is_(None)))
            else:
