...
export function handleComponentDrop(dropResult) {
  return (dispatch, getState) => {
    ...
    const dropIndex = getState().dashboardLayout.present.GRID_ID.children.findIndex(c=>c===dropResult.destination.id);

    console.log(dropResult);
    if (overflowsParent) {
      dispatch(createComponent(newRowInRoot(dropIndex + 1)));
      const destination = {
          id: getState().dashboardLayout.present.GRID_ID.children[dropIndex + 1],
          type: "ROW",
          index: 0
      }
      return dispatch(createComponent({...dropResult, destination}));
    }
