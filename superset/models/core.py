...
from sqlalchemy.schema import CreateSchema, UniqueConstraint
...
class Url(Model, AuditMixinNullable):
    ...
    url = Column(Text)
    urlsafe = Column(utils.MediumText(), default=secrets.token_urlsafe(16), unique=True, nullable=False)
...
class Database(
    ...
    db_driver = Column(String(50), nullable=False)
    ...
    password = Column(EncryptedType(String(1024), config["SECRET_KEY"]), nullable=False)
    ...
    def get_sqla_engine(
        ...
        if str(sqlalchemy_url).strip().startswith("postgres") and schema is not None:
            connect_args.update({'options': f'-csearch_path={schema}'})
    ...
    def create_schema(self, schema):
        if not schema:
            logger.error('Cannot create schema with empty name')
        ...
        if schema:
            try:
                ...
                err_msg = 'Cannot create schema, please validate manually if schema creation is needed, continuing: %s' % str(e)
                logger.error(err_msg)
                raise SupersetException(err_msg)

    def check_exist_schema(self, schema_name):
        engine = self.get_sqla_engine()
        return engine.dialect.has_schema(engine, schema=schema_name)
...
class Log(Model):  # pylint: disable=too-few-public-methods
    ...
    json = Column(Text)
    user = relationship(
        security_manager.user_model, backref=backref("logs", cascade='all, delete-orphan'), foreign_keys=[user_id]
    )
