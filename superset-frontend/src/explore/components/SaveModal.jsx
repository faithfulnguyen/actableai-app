...
class SaveModal extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dashboards: [],
      alert: null,
      action: (props.slice && props.can_overwrite) ? 'overwrite' : 'saveas',
      addToDash: 'noSave',
  ...
  componentDidMount() {
    this.props.actions
      .fetchDashboards(this.props.userId, this.props.hasAdminRole)
      .then(() => {
  ...
  saveOrOverwrite(gotodash) {
    ...
    if (sliceParams.action === 'saveas') {
      ...
      if(!this.props.slice?.slice_name){
        this.setState({ alert: t('Please enter a chart name') });
        return;
      }
  ...
  viewDashboard(canNotSaveToDash) {
    return (
      <div>
        <br />
        <hr />
  ...
  buttonDashboard(canNotSaveToDash) {
    return (
      <Button
        type="button"
        id="btn_modal_save_goto_dash"
        className="btn btn-primary pull-left gotodash"
        style={{ background: '#198ace' }}
    ...
    return (
      <Modal show onHide={this.props.onHide} bsStyle="large">
        ...
        <Modal.Body>
          ...
          {env.DASHBOARD_FEATURE === 'true' &&
            this.viewDashboard(canNotSaveToDash)}
        </Modal.Body>
        ...
        <Modal.Footer>
          ...
          {env.DASHBOARD_FEATURE === 'true' &&
            this.buttonDashboard(canNotSaveToDash)}
        </Modal.Footer>
      </Modal>
    );
  }
...
function mapStateToProps({ explore, saveModal }) {
  return {
    ...
    hasAdminRole: explore.has_admin_role,
