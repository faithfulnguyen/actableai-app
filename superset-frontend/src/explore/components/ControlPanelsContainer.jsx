...
class ControlPanelsContainer extends React.Component {
  ...
  renderControl(name, config, lookupControlData) {
    ...
    const { validationErrors, provideFormDataToProps, shouldDisplayFunc } = controlData;

    const shouldDisplay = (controls && shouldDisplayFunc) ? shouldDisplayFunc(controls) : true;
  ...
  renderControlPanelSection(section) {
    ...
    const hasErrors = section.controlSetRows.some(rows =>
      rows.some(
        s =>
          controls[s] &&
          controls[s].shouldDisplayFunc?.(controls) &&
          controls[s].validationErrors &&
          controls[s].validationErrors.length > 0,
      ),
    );

    const controlSetRows = section.controlSetRows.map((controlSets, i) => (
      <ControlRow
        key={`controlsetrow-${i}`}
  ...
  render() {
    ...
    const customTabsToRender = [];
    allSectionsToRender.forEach(section => {
      if (section.customTab) {
        customTabsToRender.push(section);
        return;
      }
      if(section.controlSetRows.some(rows =>
          rows.some(
            control =>
    ...
    return (
      <div className="scrollbar-container controlPanel" style={{ position: 'sticky', top: '153px' }}>
        <div className="scrollbar-content" style={{ pointerEvents: 'all' }}>
          {this.props.exploreState.isDatasourceLoading ? <Loading /> : (
            <>
              {this.props.alert && (
              ...
              <Tabs id="controlSections">
                ...
                {customTabsToRender.map(tab => {
                  const hasErrors = customTabsToRender?.controlSetRows?.some(([controlName]) => {
                    return !!Object.keys(this.props.controls[controlName].errors).length;
                  })
                  
                  return (
                    <Tab key={tab.label} eventKey={`tab-${tab.label}`} title={tab.label} className={hasErrors ? 'text-danger' : ''}>
                      {this.renderControlPanelSection(tab)}
                    </Tab>
                  )
                })}
